...
export default function RequestDetails() {
  ...
  useEffect(() => {
    ...
    const unsubRequest = onSnapshot(doc(db, 'requests', id), (docSnap) => {
      setRequest({ id: docSnap.id, ...docSnap.data() });
      setLoading(false);
    });

    // Listen to the answers for this request in real time
    const answersQuery = query(collection(db, 'answers'), where('requestId', '==', id));
